...
//!
//! Native mining ([`crate::solver`]) grinds the FireCash header's own nonce. Merged
//! mining instead proves the FireCash block's proof-of-work via a **parent** kHeavyHash
//! block (the Kaspa side of Option-2 dual acceptance): the parent's coinbase commits to
//! the FireCash block hash `H_fc`, and the parent's kHeavyHash clears FireCash's target.
...
/// Build an [`AuxPow`] proof for the FireCash block `fc_header` (whose hash is `H_fc`
...
/// kHeavyHash — against the *FireCash* target — until it clears it. Returns `None` if
...
///
/// Returns the (possibly nonce-adjusted) FireCash header alongside the proof: to make
...
pub fn build_aux_pow(fc_header: &Header, threads: usize, stop: &AtomicBool) -> Option<(Header, AuxPow)> {
    ...
    // Parent coinbase (leaf 0) carrying the FireCash commitment in the miner-writable slot.
    ...
    parent.bits = fc_header.bits; // grind against the FireCash target
